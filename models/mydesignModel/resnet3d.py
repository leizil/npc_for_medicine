...
class ResNet3d(nn.Module):
    ...
    def __init__(self,
        ...
        self.in_plaines=block_inplanes[0]
        self.no_max_pool=no_max_pool
        self.conv1=nn.Conv3d(n_input_channels,self.in_plaines,
                             kernel_size=(conv1_t_size,7,7),
                             stride=(conv1_t_stride,2,2),
                             padding=(conv1_t_size//2,3,3),
                             bias=False)
        self.bn1=nn.BatchNorm3d(self.in_plaines)
        self.relu=nn.ReLU(inplace=True)
    ...
    def _make_layer(self,block,planes,blocks,shortcut_type,stride=1):
        downsample=None
        if stride !=1 or self.in_plaines!=planes*block.expansion:
            if shortcut_type == 'A':
                ...
                downsample=nn.Sequential(
                    conv1x1x1(self.in_plaines,planes*block.expansion,stride=stride),
                    nn.BatchNorm3d(planes*block.expansion)
                )
        layers = []
        layers.append(block(in_channels=self.in_plaines,
                            channels=planes,
                            stride=stride,
                            downsample=downsample))
        self.in_plaines = planes * block.expansion
        for i in range(1,blocks):
            layers.append(block(self.in_plaines,planes))
